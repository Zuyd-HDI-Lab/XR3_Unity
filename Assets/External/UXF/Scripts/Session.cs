...
namespace UXF
{
    ...
    public class Session : MonoBehaviour, IExperimentUnit, IDataAssociatable
    {
        ...
        /// <summary>
        /// Enable to track objects on session level instead of trial level. This creates one output file per object, and each file contains the data between session start and end.
        /// </summary>
        [Tooltip("Enable to track objects on session level instead of trial level. This creates one output file per object, and each file contains the data between session start and end.")]
        [Reorderable]
        public bool TrackObjectsOnSessionLevel = false;
                ...
                foreach (var tracker in trackedObjects)
                {
                    ...
                    catch (NullReferenceException)
                    {
                        Utilities.UXFDebugLogWarning(
                            "An item in the Tracked Objects field of the UXF session if empty (null)!");
                    }
                }
            }
        ...
        public void End()
        {
            if (hasInitialised)
            {
                ...
                if (TrackObjectsOnSessionLevel && saveData)
                {
                    // TODO save object tracking
                    // check no duplicate trackers
                    var duplicateTrackers = trackedObjects.Where(tracker => tracker != null)
                        .GroupBy(tracker => tracker.DataName)
                        .Where(g => g.Count() > 1)
                        .Select(y => y.Key)
                        .ToList();

                    if (duplicateTrackers.Any()) throw new InvalidOperationException(
                        $"Two or more trackers in the Tracked Objects field in the Session Inspector have the following object name and descriptor pair, please change the object name fields on the trackers to make them unique: {string.Join(",", duplicateTrackers)}");
                    ...
                    foreach (var tracker in trackedObjects)
                    {
                        try
                        {
                            ...
                            // TODO check if this just works, trial uses a different format
                            SaveDataTable(tracker.Data, tracker.DataName, dataType: UXFDataType.SessionTrackers);
                        }
                        catch (NullReferenceException)
                        {
